...
PATH_MOVESENSE   = BASE / "movesense"/ f"{REC_ID}"
...
path_disk = Path(r"D:\recordings")
path_timestamps = Path(r"D:\recordings") / REC_ID / "timestamps.csv"
ts_df_grid = pd.read_csv(path_timestamps)
ts_df_grid = ts_df_grid.set_index("frame")

# Handle old and new recordings' timestamp.csv format
if "depth_timestamp" in ts_df_grid.columns:
    depth_col = "depth_timestamp"
elif "timestamp" in ts_df_grid.columns:
    depth_col = "timestamp"
else:
    raise KeyError(
        f"Cannot find depth timestamp column in {path_timestamps}.\n"
        f"  Available columns: {ts_df_grid.columns.tolist()}"
    )
grid_timestamps = ts_df_grid.loc[grid_frames, depth_col].values
