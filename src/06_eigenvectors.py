...
if not INPUT_PATH.exists():
    ...
    if not valid:
        print("No valid candidates found — using highest score overall")
    ...
    # Panel 1: Normalized offset time domain signals
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple']
    for i, c in enumerate(candidates):
        signal_norm = (c['signal'] - np.mean(c['signal'])) / np.std(c['signal'])
        time_axis_panel1 = np.arange(len(c['signal'])) / FS
        alpha = 1.0 if c == best else 0.4
        lw = 2.0 if c == best else 1.0
        label = f"{c['label']} ({c['bpm']:.0f} BPM)" + ("BEST" if c == best else "")
        axes[0].plot(time_axis_panel1, signal_norm + i * 3,
                     color=colors[i], alpha=alpha, linewidth=lw, label=label)
    axes[0].set_xlabel("Time (s)")
    axes[0].set_ylabel("Normalised amplitude (offset)")
    axes[0].set_title("Top 5 eigenvector projections (normalised, offset for clarity)")
    ...
    print(f"Looking for: {hr_file}")
    print(f"Exists: {hr_file.exists()}")
    ...
    if hr_file.exists():
        ...
        print(f"\n No heartRate.json found at {hr_file}")