...
WINDOW_SEC = 15.0
STRIDE_SEC = 1.0
GAMMA = 0.1 # <-- ΑΛΛΑΓΗ 1: Αλλαγή από 0.1 σε 0.02 σύμφωνα με το paper

# Κατώφλι Ποιότητας Σήματος (Signal Quality Index)
SQI_THRESHOLD = 0.0 # <-- ΠΡΟΣΘΗΚΗ: Απορρίπτει παράθυρα με score μικρότερο από αυτό
...
def process_window(D_win, fs, window_idx=None):
    """
    RPCA → eigendecomposition → score top eigenvectors → narrow-band refine
    the winning candidate. Returns dict with refined BPM, or None.
    """
    if D_win.shape[0] < 32:
        return None

    # ==========================================================
    # 1. SPATIAL WEIGHTING (Επιλογή των Top 14 κελιών)
    # ==========================================================
    energies = []
    ...
    for i in range(D_win.shape[1]):
        sig_cell = D_win[:, i]
        # Απλό detrending για να φύγει η αργή ολίσθηση της αναπνοής
        sig_cell = sig_cell - np.mean(sig_cell) 
        
        f, pxx = welch(sig_cell, fs=fs, nperseg=nperseg, noverlap=nperseg // 2)
        # Υπολογισμός ενέργειας ΜΟΝΟ στην καρδιακή ζώνη
        cardiac_band = (f >= HR_MIN_BPM / 60.0) & (f <= HR_MAX_BPM / 60.0)
        energies.append(np.sum(pxx[cardiac_band]))
        
    # Βρίσκουμε τα indices των 14 κελιών με τη μεγαλύτερη καρδιακή ενέργεια
    top_14_indices = np.argsort(energies)[-14:]
    
    # Το νέο μας D_win έχει πλέον μόνο 14 στήλες αντί για 28!
    D_win_selected = D_win[:, ORACLE_CELLS]
    print(f"DEBUG: ORACLE_CELLS={ORACLE_CELLS}, n={len(ORACLE_CELLS)}")
    # ==========================================================

    # Scale to unit operator norm (Χρησιμοποιούμε το D_win_selected)
    # Scale to unit operator norm
    ...
    try:
        # Τρέχουμε το RPCA μόνο στα 5 τέλεια κελιά
    ...
    # ==========================================================
    # 2. SIGNAL QUALITY INDEX (Απόρριψη θορύβου)
    # ==========================================================
    if best['score'] < SQI_THRESHOLD:
        if window_idx is not None and window_idx in (0, 10, 30):
            print(f"  [w{window_idx}] REJECTED: Low SQI score ({best['score']:.2f}) < {SQI_THRESHOLD}")
        return None
    # ==========================================================

    # ----- NARROW-BAND REFINEMENT (paper IV.D) -----
