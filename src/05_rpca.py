...
#GAMMA = 1 / np.sqrt(len(D))  # adaptive — for ~970 frames ≈ 0.032
...
if not INPUT_PATH.exists():
    ...
    rpca_meta = {
        "gamma": GAMMA,
        "rank": int(np.linalg.matrix_rank(X_low_rank)),
        "nnz_sparse": int(np.sum(np.abs(S_sparse) > 1e-12))
    }
    with open(OUTPUT_DIR / f"rpca_meta_{REC_ID}.json", 'w') as f:
        json.dump(rpca_meta, f, indent=2)
