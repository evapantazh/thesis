...
SQI_PERCENTILE = 25
MIN_WINDOWS_AFTER_SQI = 5  # safety: if SQI drops below this, fall back
...
OUTPUT_DIR = Path(r"C:\Projects\thesis\data\PULSE_files_SQI")
...
def process_window(D_win, fs, window_idx=None):
    ...
    return {
        ...
        'pulse_signal': best['signal'].tolist(),   # <-- ΠΡΟΣΘΗΚΗ
        'pulse_fs': fs,
    }
...
# ============================================================
# SQI FILTER — drop bottom SQI_PERCENTILE% of windows by score
# ============================================================
...
if SQI_PERCENTILE > 0 and n_before_sqi >= MIN_WINDOWS_AFTER_SQI * 2:
    ...
    # Safety: if SQI is too aggressive, fall back to keeping all
    if len(filtered) >= MIN_WINDOWS_AFTER_SQI:
        ...
        print(f"SQI filter         : kept {len(window_results)}/{n_before_sqi} "
              f"windows (threshold={sqi_threshold:.3f})")
    else:
        sqi_applied = False
        sqi_threshold_value = None
        print(f"SQI filter         : skipped (would leave only {len(filtered)} "
              f"windows, below safety floor of {MIN_WINDOWS_AFTER_SQI})")
else:
    sqi_applied = False
    sqi_threshold_value = None
    print(f"SQI filter         : disabled (PERCENTILE={SQI_PERCENTILE}, "
          f"n_windows={n_before_sqi})")
